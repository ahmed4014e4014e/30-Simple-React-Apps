...
export default function AnimatedLoginForm() {
  ...
  return (
    <LoginContainer className={rotate.animation}>
      <FormAnimation background={rotate.background} borderColor={rotate.borderColor} className={rotate.animation}>
      </FormAnimation>

      <form style={formStyle} className='card m-1 m-auto' onSubmit={handleSubmit}>
        <h2 className="subtitle my-2">Login</h2>
        <FormGroup labelText="Email" inputType="email" reference={email} />
        <FormGroup labelText="Password" inputType="password" reference={password} />
        <div className="form-group">
          <Button btnClass={"btn-block"} text='Submit' />
          <small className="text-accept">Don't have an account</small>
          <label className="mt-1 fw-200 text-right" style={{ textDecoration: "underline", cursor: "pointer" }}>
            Register
          </label>
        </div>
      </form>
